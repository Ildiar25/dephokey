...
class CreditCardWidget(ft.Card):
    def __init__(
        ...
        self.card_alias = ft.Text(
            (
                ...
                if self.creditcard.alias
                else "Alias tarjeta de crédito"
            ),
            font_family="AlbertSansB",
            size=18,
            color=titleCreditcardWidgetColor,
        )
        self.card_number = ft.Row(
            spacing=18,
            expand=True,
            controls=[
                ft.Text(group, font_family="IcelandR", color=tertiaryTextColor, size=20)
                for group in textwrap.wrap(
                    decrypt_data(self.creditcard.encrypted_number), width=4
                )
            ],
        )
        self.card_cvc = ft.Text(
            mask_number(decrypt_data(self.creditcard.encrypted_cvc)),
            font_family="AlbertSansB",
            size=16,
        )
    ...
    def __flip_card(self, _: ft.ControlEvent) -> None:
        self.style = (
            CreditCardStyle.BACK
            if self.style == CreditCardStyle.FRONT
            else CreditCardStyle.FRONT
        )
        self.__update_face()
        self.update()
