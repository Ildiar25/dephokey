...
class ResultsPage(ft.Column):
    ...
    def update_content(self) -> None:
        self.sites = (
            ...
            .filter(Site.name.like(f"%{self.user_input}%"))
            .all()
        )
        self.creditcards = (
            session.query(CreditCard)
            .filter_by(user_id=self.user.id)
            .filter(CreditCard.alias.like(f"%{self.user_input}%"))
        )
        self.notes = (
            session.query(Note)
            .filter_by(user_id=self.user.id)
            .filter(Note.title.like(f"%{self.user_input}%"))
        )
