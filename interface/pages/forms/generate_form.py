...
class GenerateForm(BaseForm):
    def __init__(self, title: str, page: ft.Page, style: FormStyle) -> None:
        ...
        self.submit_button = CustomElevatedButton(
            name="Generar", style=ButtonStyle.DEFAULT
        )
    ...
    def __update_appearance(self):
        match self.style:
            case FormStyle.PASSWORD:
                ...
                self.number_input = CustomTextField(
                    width=86, max_length=2, input_filter=ft.NumbersOnlyInputFilter()
                )
                self.fields = [
                    CustomSwitch(
                        title="Minúsculas",
                                ...
                                ft.Text(
                                    ...
                                    spans=[
                                        ft.TextSpan(
                                            ...
                                            "colocar un máximo de 30 o un mínimo de 7.",
                                            style=ft.TextStyle(
    ...
    def __generate_password(self, _: ft.ControlEvent) -> None:
        ...
        if self.number_input.value:
            try:
                ...
                password_length = 12
                log.error(
                    f"'{self.number_input.value}' no es un número válido.", text_number
                )
    ...
    def __update_dictionary(self) -> str:
        ...
        if self.fields[0].get_value():
            dictionary += ascii_lowercase
        if self.fields[1].get_value():
            dictionary += ascii_uppercase
        if self.fields[2].get_value():
            dictionary += digits
        if self.fields[3].get_value():
    ...
    def toggle_submit_button_state(self, cursor: ft.ControlEvent) -> None:
        if cursor and any(
            (filter(lambda switch: switch.get_value(), self.fields))
        ):  # Gets bool value from each switch
