...
class CreditCardForm(BaseForm):
    def __init__(
        ...
        self.cc_alias = CustomTextField(
            hint_text="Agrega un alias", on_change=self.__update_field_inputs
        )
    ...
    def __update_creditcard(self, _: ft.ControlEvent) -> None:
        ...
        new_alias = (
            self.cc_alias.value.capitalize().strip()
            if self.cc_alias.value
            else "Alias tarjeta"
        )
        new_holder = (
            self.cc_holder.value.title().strip()
            if self.cc_holder.value
            else self.user.fullname
        )
    ...
    def __add_creditcard(self, _: ft.ControlEvent) -> None:
        ...
        new_card = CreditCard(
            ...
            new_number,
            new_cvc,
            datetime.strptime(new_date, "%m/%y"),
            self.user,
            new_alias,
        )
        ...
        self.snackbar.change_style(
            msg=f"¡'{new_alias}' añadida!", style=SnackbarStyle.SUCCESS
        )
